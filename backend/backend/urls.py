...
router.register(r'release_task',ReleaseContextViewSet)
...
API_V1 = [
    ...
    url(r'^user_info/(?P<jwt>.*)$',UserView.as_view(),name='user_info'),  # 获取单个用户信息、修改某个用户信息、删除某个用户信息
    ...
    url(r'^context_info/(?P<jwt>.*)$',ContextView.as_view(),name='context_info'), # 获取所有句子、增加句子
    url(r'^context_info/(?P<pk>\d+)/(?P<jwt>.*)',ContextView.as_view(),name='context_info'), # 根据句子id获取句子
    url(r'^task_finish/(?P<jwt>.*)',TaskFinishView.as_view(),name='task_finish'),#根据用户id和cid获取完成情况
    ...
    url(r'^voices_info/(?P<cid>\d+)/(?P<jwt>.*)$',VoiceView.as_view(),name='voices_info'),
    ...
    url(r'^internal/user_info/$',InternalUserView.as_view(),name='internal_user'),# 获取所有用户信息、增加用户信息
    url(r'^internal/user_info/(?P<pk>.*)$',InternalUserView.as_view(),name='internal_user'),# 获取所有用户信息、增加用户信息
    url(r'^internal/task_finish/$',InternalTaskFinishView.as_view(),name='internal_task_finish'), # 获取所有的任务完成情况，客户端应该用不上
    url(r'^internal/task_finish/(?P<cid>\d+)/(?P<wx_number>.*)',InternalTaskFinishView.as_view(),name='internal_task_finish'),#根据用户id和cid获取完成情况
    url(r'^internal/task_finish/(?P<wx_number>.*)$',InternalTaskFinishView.as_view(),name='internal_task_finish'), # 获取某人完成任务的情况
]
